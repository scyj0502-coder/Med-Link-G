...
def test_discover_latest_images_from_html(monkeypatch) -> None:
    html = """
    <html>
      <body>
    ...
    assert [item.page_number for item in images] == [1, 2]
    assert all(item.source_month == "2026-05" for item in images)
    assert images[0].url == "http://www.tsmh.org.tw/sites/images-1003/202605-1.jpg"
